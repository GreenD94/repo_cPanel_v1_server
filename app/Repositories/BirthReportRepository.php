...
class BirthReportRepository extends BaseRepository
{
    ...
    public function getCases(): array
    {
        ...
        if ($user->hasRole('Doctor')) {
            $cases = PatientCase::where('tenant_id', Auth::user()->tenant_id)->with('patient.patientUser')->where('doctor_id', '=', $user->owner_id)->get()->where(
                'status',
                '=',
                1
            );
        } else {
            $cases = PatientCase::where('tenant_id', Auth::user()->tenant_id)->with('patient.patientUser')->get()->where('status', '=', 1)->sort();
        }
    ...
    {
        /** @var Doctor $doctors */
        $doctors = Doctor::with('doctorUser')->get()->where('tenant_id', Auth::user()->tenant_id)->where('doctorUser.status', '=', 1)->pluck('doctorUser.full_name', 'id')->sort();
