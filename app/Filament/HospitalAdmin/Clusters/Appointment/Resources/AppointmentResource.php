...
class AppointmentResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        ...
        $table = $table->modifyQueryUsing(function ($query) {
            $query->where('tenant_id', auth()->user()->tenant_id)->where('id', '!=', auth()->user()->id)->whereNotIn('is_completed', [AppointmentModel::STATUS_IN_QUEUE, AppointmentModel::STATUS_CHECK_IN]);
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('opd_date')
                    ...
                    ->sortable()
                    ->formatStateUsing(
                        fn ($state) => '<div class="text-center">'.Carbon::parse($state)->format('g:i A').'</div>'.Carbon::parse($state)->format('jS M, Y')
                    )
                    ->badge()
                    ->html(),
                TextColumn::make('is_completed')
                    ->label(__('messages.common.status'))
                    ->badge()
                    ->view('tables.columns.hospitalAdmin.appointment_status'),
            ])
            ->recordUrl(null)
            ->recordAction(null)
            ->filters([
